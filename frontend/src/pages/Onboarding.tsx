...
export default function Onboarding() {
    ...
    const [funMessage, setFunMessage] = useState(STEP_DEFAULTS[1]);
    ...
    const toggleSkill = (skill: string) => {
        setSelectedSkills((prev) => {
            ...
            if (!prev.includes(skill)) {
                showFun(`${skill} added to your daily loop`);
            }
    ...
    const savePreferences = async () => {
        ...
        try {
            ...
            const updatedPreferences = user.preferences.map((p) => ({
                ...p,
                enabled: selectedSkills.includes(p.skill),
                difficulty
            }));
            ...
            <div className="relative z-10 w-full max-w-lg rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl p-8 space-y-8 shadow-[0_40px_120px_rgba(0,0,0,0.6)] font-geist">
                            ...
                            {SKILLS.map((skill) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`px-4 py-3 rounded-lg border transition cursor-pointer ${active
                        ...
                        <div className="flex gap-3">
                            {DIFFICULTIES.map((level) => (
                                <button
                                    ...
                                    className={`flex-1 py-3 rounded-lg border transition cursor-pointer ${difficulty === level
                                        ? "bg-white text-black border-white"
                                        : "bg-white/5 border-white/15 text-white hover:border-white/40"
                                        }`}
                                >
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex items-center justify-between border border-white/10 rounded-lg px-4 py-3">
                            ...
                            <button
                                ...
                                className={`w-12 h-6 rounded-full transition relative cursor-pointer ${placementMode ? "bg-white" : "bg-white/20"
                                    }`}
                            >
                                <span
                ...
                <div className="flex justify-between gap-3">
                    ...
                    {step < 3 ? (
                        ...
                        >
                            {loading ? "Starting Zero..." : "Enter Zero"}
                        </button>
                    )}
                </div>
            </div>
